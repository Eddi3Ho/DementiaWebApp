...
class Auth extends CI_Controller
{
    ...
    private function _login()
    {
        ...
        if ($users && password_verify($user_password, $users['user_password'])) {
            ...
            // Check if email notification is enabled for the user
            if ($users['email_notification'] == 1) {
                //send email
                $to = $this->session->userdata('user_email');
                $from = 'laualex64@gmail.com';
                $subject = 'Dementia Web Notification';
                $message = 'Dear ' . $users['user_fname'] . ' ' . $users['user_lname'] . ',' . PHP_EOL . PHP_EOL . 'Welcome to Dementia App! Thank you for logging in to our platform.' . PHP_EOL . PHP_EOL . 'Please remember to complete the reading tasks and take the quiz questions to improve your dementia knowledge.' . PHP_EOL . PHP_EOL . 'Best regards,' . PHP_EOL . 'Dementia App Team';

                if (send_email_via_mailjet($to, $from, $subject, $message, $users['user_fname'] . ' ' . $users['user_lname'])) {
                    echo "Email sent!";
                } else {
